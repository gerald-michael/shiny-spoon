...
export interface CeasarProps {
    title: string,
    id: string,
    algorithm_key: any,
}
export default function Ceasar(props: CeasarProps) {
    ...
    const handleChange = (event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
        if (parseInt(event.target.value) > 0 && parseInt(event.target.value) <= 26) {
    ...
    return (
        <Paper elevation={2} sx={{}}>
            ...
            <TextField type="number" label="Rotation" sx={{ marginBottom: 1 }} required onChange={handleChange} value={key} />
        </Paper>
    )
}
